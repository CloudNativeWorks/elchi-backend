...
func (h *Client) processClientWithSafeguards(ctx context.Context, requestDetails models.RequestDetails, client models.ServiceClients, op models.OperationClass, processor processor.CommandProcessor) (any, error) {
	...
	// Health check before sending command to prevent stream corruption
	h.logger.Debugf("🔍 DEBUG: Starting health check for client %s", client.ClientID)
	if !h.Service.IsConnectionHealthy(client.ClientID) {
		h.logger.Warnf("🔍 DEBUG: Client %s connection unhealthy, skipping command", client.ClientID)
		return nil, fmt.Errorf("client connection unhealthy: %s", client.ClientID)
	}
	
	h.logger.Debugf("Client %s passed health check, proceeding with command", client.ClientID)
	
	// Send command with existing logic
