...
func (p *UnDeployResponser) notifyControlPlaneUndeploy(serviceName, projectName, downstreamAddress string) error {
	...
	p.Logger.Debugf("Calling NotifyUndeploy RPC for nodeID: %s", nodeID)
	_, err = (*p.XDSHandler.PokeService).NotifyUndeploy(ctxOut, request)

	if err != nil {
		p.Logger.Errorf("NotifyUndeploy RPC call failed for nodeID %s: %v", nodeID, err)
	} else {
		p.Logger.Debugf("NotifyUndeploy RPC call completed successfully for nodeID: %s", nodeID)
	}
