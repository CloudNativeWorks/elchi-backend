...
func (custom *AppHandler) GetCustomResourceListWithSearch(ctx context.Context, _ models.ResourceClass, requestDetails models.RequestDetails) (any, error) {
	...
	if requestDetails.Search != "" {
		...
		if len(searchFilter) > 0 {
			// Combine base filters with search filter using $and
			filters = bson.M{
				"$and": []bson.M{
					filters,
					{"$or": searchFilter},
...
// buildSearchFilter creates MongoDB $or filter for searching across multiple fields
