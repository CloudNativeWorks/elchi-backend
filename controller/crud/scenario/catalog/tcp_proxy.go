...
var TcpProxyDefinition = models.ComponentDefinition{
	...
	AvailableFields: []models.AvailableField{
		...
		},
		{
			...
			Description:          "Access logger configuration for TCP proxy - select from access logs defined in this scenario",
			Type:                 models.FieldTypeSelect,
			RequiredForCreation:  false,
			RequiredForExecution: false,
			Connected: &models.ConnectedField{
				ComponentTypes: []string{"access_log_file", "access_log_stdout", "access_log_fluentd"},
				FieldName:      "name",
			},
			HasMetadata:   true,
