...
func (eg *EndpointGenerator) Generate(instance models.ComponentInstance) (any, error) {
	...
	case "discovery":
		...
		roles := eg.getValueFromNestedSelection(endpointConfig, "roles")
		...
		if roles != nil {
			...
			switch roleStr {
			case "master":
				rolesSlice = []string{"master"}
			case "worker":
				rolesSlice = []string{"worker"}
			case "all":
				rolesSlice = []string{"master", "worker"} // All nodes = master + worker
			case "":
				// Empty means default to worker
				rolesSlice = []string{"worker"}
			default:
				// Single role value
				rolesSlice = []string{roleStr}
			}
		}
