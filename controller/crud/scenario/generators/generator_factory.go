...
func (gf *GeneratorFactory) CreateGenerator(componentType string) (ComponentGenerator, error) {
	...
	case "access_log_file":
		return NewFileAccessLogGenerator(gf.project, gf.version, gf.user), nil
	case "access_log_stdout":
		return NewStdoutAccessLogGenerator(gf.project, gf.version, gf.user), nil
	case "access_log_fluentd":
