...
func (hg *HCMGenerator) Generate(instance models.ComponentInstance) (any, error) {
	...
	if routeConfigSelection != nil && routeConfigSelection.SelectedChoice == "rds" {
		...
		if vhostSelection != nil && vhostSelection.SelectedChoice == "vhds" {
			// Use VHDS (Virtual Host Discovery Service)
			routeConfig["vhds"] = map[string]any{
				"config_source": map[string]any{
					"ads":                  map[string]any{},
					"resource_api_version": "V3",
				},
			}
		} else if vhostSelection != nil && vhostSelection.SelectedChoice == "inline_virtual_hosts" {
	...
	// Add VHDS config discovery if VHDS is selected
	if routeConfigSelection != nil && routeConfigSelection.SelectedChoice == "inline" {
		for _, subField := range routeConfigSelection.SubFields {
			if subField.FieldName == "virtual_host_config" && subField.NestedSelection != nil {
				vhostSelection := subField.NestedSelection
				if vhostSelection.SelectedChoice == "vhds" {
					// Get VHDS config source name
					var vhdsName string
					for _, vhdsSubField := range vhostSelection.SubFields {
						if vhdsSubField.FieldName == "vhds_config_source" && vhdsSubField.Value != nil {
							if vhdsConfigObj, ok := vhdsSubField.Value.(map[string]any); ok {
					...
					if vhdsName != "" {
						...
						// Add VHDS config discovery entry
						vhdsConfigEntry := map[string]any{
							"name":           vhdsName,
							"gtype":          gtypeVHDS.String(),
							"priority":       0,
							"category":       gtypeVHDS.Category(),
							"canonical_name": gtypeVHDS.CanonicalName(),
							"parent_name":    inlineRouteName,
						}
						configDiscoveryEntries = append(configDiscoveryEntries, vhdsConfigEntry)
					}
				}
				break
			}
