...
func (t *AppHandler) GetScenarios(project string, reqDetails models.RequestDetails) (*models.ScenarioListResponse, error) {
	...
	filter := bson.M{
		"$or": []bson.M{
			...
			{"project": bson.M{"$exists": false}},       // Legacy scenarios without project field
		},
	}
...
func (t *AppHandler) ExecuteScenario(request models.ExecuteScenarioRequest, reqDetails models.RequestDetails) ([]map[string]interface{}, error) {
	...
	if errors := t.ValidateComponentRulesWithContext(componentsToProcess, ValidationContextExecution); len(errors) > 0 {
		...
		if request.Managed && len(createdResources) > 0 {
	...
	for i, componentInstance := range componentsToProcess {
		...
		// Save to database if managed flag is true
		if request.Managed {
			fmt.Printf("[%d/%d] Saving %s to database: %s\n", i+1, len(componentsToProcess), componentInstance.Type, componentInstance.Name)
			...
			fmt.Printf("NOT saving to database - Managed: %t, Type: %s\n", request.Managed, componentInstance.Type)
		}
		
		generatedResources = append(generatedResources, document)
	}
	
	fmt.Printf("✅ Successfully created %d resources\n", len(createdResources))
...
func convertDocumentToResourceClass(document map[string]interface{}) (models.ResourceClass, error) {
	...
	// Debug - check if values are preserved after conversion
	if resource.GetGeneral().Type == "network_filter" {
		fmt.Printf("🔍 After conversion - DBResource General Type: %s\n", resource.GetGeneral().Type)
		
		resourceSection := resource.GetResource()
		fmt.Printf("🔍 resource.GetResource() result: %v\n", resourceSection)
		
		if resourceSection != nil {
			if resourceSectionMap, ok := resourceSection.(map[string]interface{}); ok {
				fmt.Printf("🔍 resourceSectionMap keys: %v\n", getMapKeys(resourceSectionMap))
				
				// For network_filter, the typed_config is directly in resourceSectionMap
				if typedConfig, ok := resourceSectionMap["typed_config"].(map[string]interface{}); ok {
					fmt.Printf("🔍 After conversion - stat_prefix: '%v'\n", typedConfig["stat_prefix"])
					fmt.Printf("🔍 After conversion - route_config: '%v'\n", typedConfig["route_config"])
				} else if resourceData, ok := resourceSectionMap["resource"]; ok {
					if filterData, ok := resourceData.(map[string]interface{}); ok {
						fmt.Printf("🔍 filterData keys: %v\n", getMapKeys(filterData))
						
						if typedConfig, ok := filterData["typed_config"].(map[string]interface{}); ok {
							fmt.Printf("🔍 After conversion - stat_prefix: '%v'\n", typedConfig["stat_prefix"])
							fmt.Printf("🔍 After conversion - route_config: '%v'\n", typedConfig["route_config"])
						} else {
							fmt.Printf("🔍 typed_config not found or wrong type\n")
						}
					} else {
						fmt.Printf("🔍 filterData cast failed\n")
					}
				} else {
					fmt.Printf("🔍 resource key not found\n")
				}
			} else {
				fmt.Printf("🔍 resourceSectionMap cast failed\n")
			}
		} else {
			fmt.Printf("🔍 resource.GetResource() returned nil\n")
		}
	}
...
func (t *AppHandler) saveResourceToDatabaseWithID(document map[string]interface{}, reqDetails models.RequestDetails) (interface{}, error) {
	...
	// Debug log
	fmt.Printf("Converting document to ResourceClass for: %s (type: %s)\n", resource.GetGeneral().Name, resource.GetGeneral().Type)

	// Debug log before XDS SetResource
	fmt.Printf("🔍 About to save %s to XDS\n", resource.GetGeneral().Name)
	
	// Debug resource data that will be sent to validation
	if resource.GetGeneral().Type == "network_filter" {
		resourceData := resource.GetResource()
		resourceBytes, _ := json.Marshal(resourceData)
		fmt.Printf("🔍 Resource data being validated: %s\n", string(resourceBytes))
	}
	...
	if err != nil {
		fmt.Printf("❌ XDS SetResource failed for %s: %v\n", resource.GetGeneral().Name, err)
	...
	fmt.Printf("✅ XDS SetResource success for %s\n", resource.GetGeneral().Name)
	...
	for i := len(createdResources) - 1; i >= 0; i-- {
		...
		if collection != nil {
			...
			if err != nil {
				...
				fmt.Printf("    ⚠️  No document deleted for %s (ID: %v)\n", resource.Name, resource.ID)
			} else {
