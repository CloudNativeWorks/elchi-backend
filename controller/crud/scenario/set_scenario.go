	...
	for key, templateStr := range templateMap {
		if key != "managed" {
			if data, ok := scenario[key]; ok {
				// Convert data to map for manipulation
				dataMap, ok := data.(map[string]any)
				if !ok {
					sc.rollback(ctx, successfulResources, reqDetails)
					return nil, fmt.Errorf("invalid data format for key: %s", key)
				}

				// Add HCM base64 encoding for listener templates
				if key == "listener" {
					if hcmName, exists := dataMap["hcm"]; exists {
						...
						hcmJSON, err := json.Marshal(hcmConfig)
						if err != nil {
							sc.rollback(ctx, successfulResources, reqDetails)
							return nil, fmt.Errorf("failed to marshal HCM config: %w", err)
						}
						
						dataMap["hcm_base64"] = base64.StdEncoding.EncodeToString(hcmJSON)
					}
					
					// Add TCP proxy base64 encoding for TCP listeners
					if tcpProxyName, exists := dataMap["tcp_proxy"]; exists {
						...
						dataMap["tcp_proxy_base64"] = base64.StdEncoding.EncodeToString(tcpJSON)
					}
				}
...
func (sc *AppHandler) SetResource(ctx context.Context, data models.ResourceClass, reqDetails models.RequestDetails) (map[string]any, error) {
	// CRITICAL FIX: Update reqDetails.Name with actual resource name from template
	// This prevents malformed nodeID generation during validation
	if data.GetGeneral().Name != "" {
		reqDetails.Name = data.GetGeneral().Name
	}
