...
func (dh *DiscoveryHandler) HandleK8sDiscovery(c *gin.Context) {
	...
	if len(nodeRoleCounts) > 0 {
		log.Printf("Node roles summary - Masters: %d, Workers: %d, All roles: %v",
			masterCount, workerCount, nodeRoleCounts)
		log.Printf("Endpoint updates will only process %d worker nodes (masters excluded)", workerCount)
	}
...
func (dh *DiscoveryHandler) GetClusters(c *gin.Context) {
	...
	if err != nil {
		log.Printf("Failed to get clusters: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{
