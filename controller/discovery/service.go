...
func (ds *DiscoveryService) updateEndpointFromNodes(ctx context.Context, endpoint models.DBResource, nodes []NodeInfo, clusterName string) EndpointUpdateResult {
	...
	// Build new IP list from K8s nodes based on user's AddressType preference
	newIPs := []string{}
	for _, node := range nodes {
		if node.Status == "Ready" {
			// Use the AddressType specified by user in discovery config
			switch discoveryConfig.AddressType {
			case "ExternalIP":
...
func (ds *DiscoveryService) triggerAsyncSnapshotUpdate(ctx context.Context, endpoint models.DBResource, project string) {
	...
	if err != nil {
		log.Printf("Failed to initialize async system for discovery update, falling back to sync: %v", err)
	...
	if err := asyncSystem.SetPokeService(ds.pokeService); err != nil {
		log.Printf("Failed to set poke service for discovery update, falling back to sync: %v", err)
		// Fallback to synchronous processing
		ds.triggerSyncSnapshotUpdate(ctx, endpoint, project)
		return
	}
	...
	if err != nil {
		log.Printf("Failed to create preliminary discovery job, falling back to sync: %v", err)
		// Fallback to synchronous processing
		ds.triggerSyncSnapshotUpdate(ctx, endpoint, project)
		return
	}
	...
	log.Printf("Created preliminary discovery job %s for endpoint %s (node changes), analyzing dependencies in background",
...
func (ds *DiscoveryService) triggerSyncSnapshotUpdate(ctx context.Context, endpoint models.DBResource, project string) {
	...
	if processed != nil {
		log.Printf("Triggered sync snapshot update for endpoint %s, affected listeners: %v", 
...
func (ds *DiscoveryService) extractNodeIPsFromRequest(request K8sDiscoveryRequest, addressType string) []string {
	...
	for _, node := range request.Data.Nodes {
		if node.Status == "Ready" {
			// Use the AddressType specified by user
			switch addressType {
			case "ExternalIP":
				if externalIP, exists := node.Addresses["ExternalIP"]; exists && externalIP != "" {
