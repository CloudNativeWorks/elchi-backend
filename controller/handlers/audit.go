...
func (h *Handler) setResourceAuditContext(c *gin.Context, requestDetails models.RequestDetails) {
	...
	// Set save_or_publish information for XDS and Extension operations
	if (isXDS || isExtension) && requestDetails.SaveOrPublish != "" {
		audit.SetAuditSaveOrPublish(c, requestDetails.SaveOrPublish)
	}
}
