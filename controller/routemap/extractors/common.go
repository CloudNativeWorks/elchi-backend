...
// ExtractMetadataForRouteConfig extracts important metadata for route configuration
func ExtractMetadataForRouteConfig(routeConfig any) map[string]any {
	metadata := make(map[string]any)
	
	if statsPrefix := ExtractString(routeConfig, "stats_prefix"); statsPrefix != "" {
		metadata["stats_prefix"] = statsPrefix
	}
	if bufferLimit := ExtractString(routeConfig, "per_request_buffer_limit_bytes"); bufferLimit != "" {
		metadata["per_request_buffer_limit_bytes"] = bufferLimit
	}
	if validateClusters := ExtractBool(routeConfig, "validate_clusters"); validateClusters {
		metadata["validate_clusters"] = "true"
	}
	if mostSpecific := ExtractBool(routeConfig, "most_specific_header_mutations_wins"); mostSpecific {
		metadata["most_specific_header_mutations_wins"] = "true"
	}
	if maxDirectResponseBodySize := ExtractString(routeConfig, "max_direct_response_body_size_bytes"); maxDirectResponseBodySize != "" {
		metadata["max_direct_response_body_size_bytes"] = maxDirectResponseBodySize
	}
	
	return metadata
}

// ExtractMetadataForVirtualHost extracts important metadata for virtual host
func ExtractMetadataForVirtualHost(virtualHost any) map[string]any {
	metadata := make(map[string]any)
	
	if requireTLS := ExtractString(virtualHost, "require_tls"); requireTLS != "" {
		metadata["require_tls"] = requireTLS
	}
	if includeAttemptCount := ExtractBool(virtualHost, "include_request_attempt_count"); includeAttemptCount {
		metadata["include_request_attempt_count"] = "true"
	}
	if perFilterConfig := ExtractMap(virtualHost, "typed_per_filter_config"); perFilterConfig != nil {
		metadata["has_per_filter_config"] = "true"
	}
	
	return metadata
}

// ExtractMetadataForRoute extracts important metadata for route
func ExtractMetadataForRoute(route any) map[string]any {
	metadata := make(map[string]any)
	
	// Route-level timeout
	if timeout := ExtractString(route, "timeout"); timeout != "" {
		metadata["timeout"] = timeout
	}
	if idleTimeout := ExtractString(route, "idle_timeout"); idleTimeout != "" {
		metadata["idle_timeout"] = idleTimeout
	}
	if upgradeConfigs := ExtractArray(route, "upgrade_configs"); len(upgradeConfigs) > 0 {
		metadata["has_upgrade_configs"] = "true"
	}
	if metadata_val := ExtractMap(route, "metadata"); metadata_val != nil {
		metadata["has_metadata"] = "true"
	}
	if decorator := ExtractMap(route, "decorator"); decorator != nil {
		if operation := ExtractString(decorator, "operation"); operation != "" {
...
// ExtractMetadataForMatch extracts important metadata for match conditions
func ExtractMetadataForMatch(route any) map[string]any {
	metadata := make(map[string]any)
	...
	if caseSensitive := ExtractBool(matchObj, "case_sensitive"); !caseSensitive {
		metadata["case_sensitive"] = "false"
	}
	if grpcConfig := ExtractMap(matchObj, "grpc"); grpcConfig != nil {
		metadata["grpc_match"] = "true"
	}
	if tlsContext := ExtractMap(matchObj, "tls_context"); tlsContext != nil {
		metadata["has_tls_context"] = "true"
	}
	if runtime_fraction := ExtractMap(matchObj, "runtime_fraction"); runtime_fraction != nil {
		if defaultValue := ExtractString(runtime_fraction, "default_value.numerator"); defaultValue != "" {
			metadata["runtime_fraction"] = defaultValue
		}
	}
...
// ExtractMetadataForAction extracts important metadata for route actions
func ExtractMetadataForAction(route any) map[string]any {
	metadata := make(map[string]any)
	
	// Check route action
	if routeObj := ExtractMap(route, "route"); routeObj != nil {
		if timeout := ExtractString(routeObj, "timeout"); timeout != "" {
			metadata["timeout"] = timeout
		}
		if idleTimeout := ExtractString(routeObj, "idle_timeout"); idleTimeout != "" {
			metadata["idle_timeout"] = idleTimeout
		}
		if retryPolicy := ExtractMap(routeObj, "retry_policy"); retryPolicy != nil {
			if numRetries := ExtractString(retryPolicy, "num_retries.value"); numRetries != "" {
				metadata["max_retries"] = numRetries
			}
			if retryOn := ExtractString(retryPolicy, "retry_on"); retryOn != "" {
				metadata["retry_on"] = retryOn
			}
		}
		if hedgePolicy := ExtractMap(routeObj, "hedge_policy"); hedgePolicy != nil {
			metadata["has_hedge_policy"] = "true"
		}
		if maxStreamDuration := ExtractMap(routeObj, "max_stream_duration"); maxStreamDuration != nil {
			if maxStreamDurationVal := ExtractString(maxStreamDuration, "max_stream_duration"); maxStreamDurationVal != "" {
				metadata["max_stream_duration"] = maxStreamDurationVal
			}
		}
	}
	...
	if redirectObj := ExtractMap(route, "redirect"); redirectObj != nil {
		if responseCode := ExtractInt(redirectObj, "response_code"); responseCode > 0 {
			metadata["response_code"] = fmt.Sprintf("%d", responseCode)
		}
		if stripQuery := ExtractBool(redirectObj, "strip_query"); stripQuery {
			metadata["strip_query"] = "true"
		}
	}
	
	// Check direct response action
	if directResponseObj := ExtractMap(route, "direct_response"); directResponseObj != nil {
		if bodyString := ExtractString(directResponseObj, "body.inline_string"); bodyString != "" {
			metadata["response_body"] = bodyString
		}
		if bodyBytes := ExtractString(directResponseObj, "body.inline_bytes"); bodyBytes != "" {
			metadata["has_binary_body"] = "true"
		}
	}
