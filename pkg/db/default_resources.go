...
func createDefaults(ctx context.Context, context *AppContext, logger *logger.Logger) {
	...
	if err := CreateGenericUpstreamTLS(ctx, context, projectID, vrs, groupID); err != nil {
		logger.Infof("Generic upstream tls not created: %s", err)
	}
	...
	if err := CreateDefaultAccessLog(ctx, context, projectID, vrs, groupID); err != nil {
...
func CreateGenericUpstreamTLS(ctx context.Context, db *AppContext, projectID string, vers string, groupID string) error {
	gtype := models.UpstreamTLSContext
	collection := db.Client.Collection(gtype.CollectionString())
	var tls models.Resource
	if projectID == "" {
	...
	err := collection.FindOne(ctx, bson.M{"general.name": "default-upstream-tls", "general.version": vers, "general.project": projectID}).Decode(&tls)
	...
	case errors.Is(err, mongo.ErrNoDocuments):
		...
		if err != nil {
			if mongo.IsDuplicateKeyError(err) {
				db.Logger.Infof("generic upstream tls already exists: %v", err)
			} else {
				return fmt.Errorf("failed to create generic upstream tls: %w", err)
			}
		} else {
			db.Logger.Info("generic upstream tls created successfully")
		}
	case err != nil:
		return fmt.Errorf("failed to check for generic upstream tls: %w", err)
	default:
		db.Logger.Info("generic upstream tls already exists")
	}
...
func CreateDefaultAccessLog(ctx context.Context, db *AppContext, projectID string, vers string, groupID string) error {
	gtype := models.StdoutAccessLog
	collection := db.Client.Collection(gtype.CollectionString())
	var accessLog models.Resource
	if projectID == "" {
		return errstr.ErrProjectIDEmpty
	}
	err := collection.FindOne(ctx, bson.M{"general.name": "default-aclog-stdout", "general.version": vers, "general.project": projectID}).Decode(&accessLog)
	...
	case errors.Is(err, mongo.ErrNoDocuments):
		...
		// Professional log format with all important fields
		...
		defaultAccessLog := bson.M{
			"general": bson.M{
				"name":           "default-aclog-stdout",
				"version":        vers,
				"type":           gtype.Type(),
				"gtype":          gtype.String(),
				"project":        projectID,
			...
			"resource": bson.M{
				...
				"resource": bson.M{
					"log_format": bson.M{
						"text_format_source": bson.M{
							"inline_string": logFormat,
						},
					},
				},
			},
		...
		_, err = collection.InsertOne(ctx, defaultAccessLog)
		if err != nil {
			if mongo.IsDuplicateKeyError(err) {
				db.Logger.Infof("default access log already exists: %v", err)
			} else {
				return fmt.Errorf("failed to create default access log: %w", err)
			}
		} else {
			db.Logger.Info("default access log created successfully")
		}
	case err != nil:
