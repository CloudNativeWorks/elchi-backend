...
// Supports role mapping: "master" matches both "master" and "control-plane"
func NodeMatchesRoles(node NodeInfo, requiredRoles []string) bool {
	for _, requiredRole := range requiredRoles {
		for _, nodeRole := range node.Roles {
			if roleMatches(nodeRole, requiredRole) {
				return true
			}
		}
	}
...
// roleMatches checks if a node role matches a required role, including aliases
func roleMatches(nodeRole, requiredRole string) bool {
	// Direct match
