...
// DeleteController deletes a controller and its associated client mappings
func (s *ControllerRoutingService) DeleteController(ctx context.Context, controllerID string) error {
	s.logger.Infof("Deleting controller: %s", controllerID)
	...
	_, err := s.storage.GetController(ctx, controllerID)
	if err != nil {
		return fmt.Errorf("controller %s not found: %w", controllerID, err)
	}

	// Delete controller and all its associated client mappings
	if err := s.storage.DeleteController(ctx, controllerID); err != nil {
		return fmt.Errorf("failed to delete controller %s: %w", controllerID, err)
	}
